...
> 本文档按 2026-09-11 的实际代码状态维护，供后续编码助手快速定位当前实现、历史实现和验证方式。
...
- 首页统计卡的课程数量以教务网课表的去重课程名为准（`timetableCourseCount`），只有教务网没有有效结果时才回退到学在浙大课程列表。
...
- 固定尺寸应服务于对齐，不等于把内容裁掉。所有卡片都要检查长标题、窄屏和本地文库较大字号下的高度。

### 4.4 课程表、辅助栏与考试

- 课程表界面（`TimetableView`）表头改版：
  - 清空原本卡片内部居中的“浙江大学课程表（...）”静态文本，改为表头行。
  - 左侧为 **Tab 切换学期** 胶囊按钮组，支持水平顺滑滚动，点击单个 Tab 切换并优先读取本地缓存（非强制网络请求）。
  - 右侧集成操作按钮组：**导出图片**、**导出 Excel**、**刷新课表**。
  - 响应式自适应（`LayoutBuilder`）：宽屏（>= 560px）展示带文本与图标的操作按钮；窄屏（< 560px）自动收缩为紧凑的图标按钮（带 Tooltip 提示），右侧仅占约 108px，为左侧 Tab 滚动留出最大空间，避免 RenderFlex overflow。
...
- 课程表与辅助栏完全解耦：
  ...
  - 辅助栏课程详情抽屉新增“作业”Tab，复用作业卡片样式展示课程所有作业（不分类）；辅助栏中作业卡片底色与资料卡片一致（`paper`），待办作业主页保持原卡纸色（`paperCard`）。
...
- 缓存更新时间用于页面显示“数据更新于 N 分钟前”；通知两个来源分别记录更新时间和失败状态，不能因单个来源失败覆盖另一个来源。
...
- `application/agent.dart` 是当前客户端 Agent：直接调用模型，工具覆盖课程、作业、课件、测验、日程、考试、课表、通知、成绩、知识库和下载。
...
4. 页面刷新要区分首次进入、手动刷新和应用生命周期变化；不要把生命周期回调直接当成每次请求入口。
5. 课程数、考试数等统计必须注明数据源，不能把学在浙大课程列表和教务网课表混为同一份数据。
6. 资源文件改动要同步检查 `pubspec.yaml` 打包声明；不要用 fallback 空提示词掩盖资源缺失。
7. 不要执行破坏性 Git 操作（如 `git reset --hard`、覆盖用户未提交修改），除非用户明确要求。
8. 修改 `start-dev.bat` / `stop-dev.bat` 时保持 GBK + CRLF；不要用普通 UTF-8 编辑器直接保存。

## 11. 测试和验收

### Flutter

当前测试覆盖依赖兼容性、领域模型/课表、LLM URL 和请求、登录 cookie、认证协议、错误展示、SQLite/存储、作业筛选过滤、课程详情 Tab 及作业展示、课程表表头自适应/解耦/导出、设置页折叠展开等；原生测试覆盖 Windows 安全存储和 SQLite，另有真实登录手工测试。2026-09-12 扫描时 `flutter test` 为 56 项全通过；`flutter analyze` 无 error，无 warning，仅存 13 条 info 级风格/弃用提示。
