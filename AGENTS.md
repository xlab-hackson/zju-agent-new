...
- `ui/pages_v2.dart` 目前承载工作台及主要功能页；`ui/chat.dart` 是唯一在用的全局悬浮 AI 对话窗。
...
- **学业快览复合卡片重构（Academic Overview KPI Cards）**：
  - 4 张卡片维持统一 170px 固定高度、纸张底色与响应式布局（宽屏 4 列、中屏 2 列、窄屏 1 列）。
  - **卡片 1（本学期学业）**：整合课程、学分与考试，从左到右依次排列**课程数**（点击跳转至 `/courses`）、**学分数**（展示当前学期选课学分，`onTap: null` 不跳转）、**考试数**（点击跳转至 `/exams`）。
...
- **日程卡片与头像交互体验优化**：
  ...
  - **日程卡片点按交互**：日程卡片包裹 `InkWell` 点击事件（`_openEventCourse`），点击时自动通过 `_enrichCourse` 模糊对齐本地课表缓存，补充上课时间、教室与任课教师后弹出对应的课程详情抽屉（`CourseDetailSheet`）。
...
- 课程表界面（`TimetableView`）表头改版：
  ...
  - 左侧为 **Tab 切换学期** 胶囊按钮组：桌面端（宽屏）限制为表头一半宽度（`constraints.maxWidth * 0.5`），中间通过 `Spacer()` 与右侧操作按钮对齐；当学期 Tab 按钮较多时，每个按钮自适应大小（动态内边距、字号并配合 `FittedBox` 缩放），使全部按钮总宽度自适应保持在半行之内，免去横向滑动，彻底解决桌面端鼠标无法横向滑动的体验问题；窄屏（< 560px）保留剩余可用空间自适应并支持平滑水平滑动。点击单个 Tab 切换并优先读取本地缓存（非强制网络请求）。
  - 右侧集成操作按钮组：**导出图片**、**导出 Excel**、**刷新课表**。
    - 响应式自适应（`LayoutBuilder`）：宽屏（>= 620px）展示带文本与图标的操作按钮；窄屏（< 620px，含桌面极限收窄到 800px 时内容区约 584px）自动收缩为紧凑的图标按钮（带 Tooltip 提示），右侧仅占约 108px，为左侧 Tab 留出最大空间，彻底杜绝 18px RenderFlex overflow。
  ...
  - 课程表与辅助栏完全解耦：
    - 课程表下方遗留的静态课程目录已移除。
    - 课程表学期与右侧辅助栏（学期总览）学期互相独立，切换课表学期不影响辅助栏课程过滤；两路的刷新机制独立触发。
    ...
    - **全历史学期学分、教师与成绩/绩点穿透与学期隔离（Past Semesters Enrichment & Grade Badges）**：
      ...
      - **学在浙大建课状态关联与未建课课程页面承载**：将教务网课程与学在浙大课程列表进行精准/模糊对齐，已建课课程保留学在浙大课程 ID；未在学在浙大建课的课程标记 `learningZjuCreated: false`，在辅助栏展示“未建课”灰章标签；所有课程（包括未建课课程）均可点击唤起课程详情抽屉（`CourseDetailSheet`），完整展示课程基本信息（教师、教室、上课时间、学分、成绩等），其课程资料与课程作业直接展示空态，并在 Tab 栏那一行右侧常驻展示“学在浙大无此课程”小字提示，同时避免对未建课课程发起无谓网络请求。
    ...
    - **学业统计卡片集成与学期动态联动（Academic Stats Card in CourseRightPanel）**：
      - 在辅助栏学期 Tab 胶囊按钮组下方集成学业统计卡片（`_buildStatsCard`），依托 `_computePanelStats` 与教务网全量成绩库。
  ...
  - Flutter 课表使用 `ui/pages_v2.dart` 中的 `TimetableView`：7 个日期列在可用宽度内自适应，课程内容区域可滚动；去除了原有内部单元格网格边框线（每节网格线），课程卡片直接展示在背景底色上保持整洁；当前纵向仍是 `Stack`/`Positioned` 加固定 52px 节次高度，且可视网格只绘制 1–13 节，因此“纵向内容自适应”仍是后续改造项。不要把这一现状误写成已经完全自适应，也不要在修复时继续扩大固定高度掩盖溢出。
    ...
    - **课程色块点按交互**：课程色块包裹 `InkWell` 点击事件（`onSelectCourse` 回调），在工作台点击后通过 `_openTimetableCourse` 自动匹配课程列表并丰富上课时间、教室、教师属性后，无缝调起对应课程详情抽屉。
...
2. 所有异步操作先完成异步工作，再同步 `setState`/更新 provider；不要把 `async` 闭包传给 Flutter `setState`。
...
- 验证：新增页面上下文、学业快览、缓存去重/广播和更新时间回归测试；当前 Flutter 全量测试 100 项通过，`flutter analyze` 无问题。
