...
- 服务字段或构造初始化发生变化后，旧实例可能在热重载中保留。出现 `FileService._root` 为 null 的类型错误时，先执行 Hot Restart（`flutter run` 终端按大写 `R`）或停止后重新启动；不要为旧内存状态加入空目录兜底。详见 [Flutter 开发排错](apps/flutter/README.md#开发排错)。
...
- 课程表界面（`TimetableView`）表头改版：
  ...
  - 左侧为 **Tab 切换学期** 水平滚动胶囊按钮组：通过 `Expanded` 弹性承载配合 `SingleChildScrollView(scrollDirection: Axis.horizontal)`，无缝适应任意数量学期（从 2 个到 10+ 个学期）；内置 `PointerScrollEvent` 监听将纵向鼠标滚轮直接映射为平滑水平滚动，无需按住 Shift，并支持触屏、鼠标与触控板拖拽（`dragDevices`）；窄屏（< 700px）右侧操作区自动收缩为 108px 紧凑图标，配合左侧弹性区域彻底杜绝所有窄窗/极端尺寸下的 RenderFlex overflow。
  ...
  - 课程表与辅助栏结构分离与 Tab 联动同步：
    ...
    - 右侧辅助栏（`CourseRightPanel`）学期切换改版与任课老师：学期选择器采用统一 32px 紧凑高度的单行水平平滑滚动胶囊按钮组（`_HorizontalSemesterTabs`），彻底解决多学期（如 10+ 学期）在有限 240px 辅助栏宽度下挤压文字或分行导致溢出的问题；支持鼠标滚轮直接横向滚动、触控板/鼠标拖拽滑动，并在学期切换或初始进入时自动平滑居中滚动聚焦至选中项（`_scrollToSelected`）；通过重构的 `semesterDisplay` 精确识别秋学期（如 `2024-2025秋`）与冬学期（`2024-2025冬`），解决原本将秋/冬单学期误标为“秋冬”的 bug；每个课程项在课程名下方展示任课老师姓名（带 `Icons.person_outline` 紧凑图标）。
