...
- 学校信息页不使用上拉刷新，但标题栏刷新按钮会强制重新请求两个通知源；通知数据现在按一天缓存，并显示两路通知中最旧数据的更新时间。
...
- 每条缓存记录至少要能提供数据更新时间和 stale 状态；页面展示最近一次成功数据距当前的分钟数。通知的素质拓展、教务网两个来源必须独立缓存、独立刷新和独立回退。
- 首页连接状态读取凭据存在性和 `CampusSession.authStatus`，没有单独的状态探测请求；但工作台首次加载仍会调用课程/课表/作业/考试/48 小时日程加载器，实际是否联网由各数据缓存决定。模型卡片当前只表示存在来源配置，不等于实时可用性检测通过。
...
- 当前自动测试主要覆盖领域、协议、存储和少量组件兼容性，没有覆盖工作台/课表/设置/聊天窗在多个实际宽度下的 widget golden 或 overflow 回归；这些仍需真机/窗口尺寸手工验收，最好补充可重复的 widget 测试。
...
- 反复进入首页不会每次强制请求校园接口。
- 每个数据页面本次运行首次进入时强制刷新一次，之后手动刷新仍强制请求最新数据；页面标题显示“数据更新于 N 分钟前”。
